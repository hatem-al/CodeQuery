...
2. Ask a question — the backend classifies intent (location / usage / architecture), optionally generates a HyDE document for deep queries, runs a hybrid BM25 + vector search with RRF fusion, grounds the prompt with a full repo file listing, then streams a GPT-4o answer back in real time
...
**Parsing:** Python AST + pattern-based chunking (JS/TS/Java/C++/Go/Rust/Ruby/PHP/C#/Swift/Kotlin/Scala) + docs and config files
...
```bash
cd backend
./venv/bin/python -m pytest   # 76 tests, no live services needed
```
...
- **Hybrid search** — BM25 + cosine similarity fused with RRF, language-neutral ranking
- **Repo overview grounding** — every prompt includes the indexed file listing, so broad questions work
